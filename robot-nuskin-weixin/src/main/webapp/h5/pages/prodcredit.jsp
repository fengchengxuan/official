...
<html class="ng-scope" ng-app="mCommerce" lang="zh">
	...
	<style class="ng-scope">
	 ...
	 table tbody tr td{display: table-cell;height:25px;color: #333;vertical-align:middle;}
	...
	<div class="tab">
		...
		<c:if test="${!empty list}">
			<table class="tablesorter" id="historyOrderTable">
		    	<thead>
		    		<tr>
			         	<th style="text-align:center;" class="header">异动类别</th>
			         	<th style="text-align:center;">调整积分</th>
			         	<th style="text-align:center;">日期</th>
			         	<th style="text-align:center;" class="header">订单编号</th>
		        	</tr>
		     	</thead>
		     	<tbody>
		     	<c:forEach items="${list}" var="prodcredit">
		     		<tr>
		     			<td>${prodcredit.transTypeDesc}</td>
		     			<td><p style="color:black;">${prodcredit.productCredit}</p></td>
		     			<td>${prodcredit.createDtmStr}</td>
		     			<td><a target="_blank" href="prodcredit!show.action?token=${token}&id=${prodcredit.referenceId}">${prodcredit.referenceId}</a></td>
		     		</tr>
		     	</c:forEach>
		     	</tbody>
		     </table>
