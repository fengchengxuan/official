...
public class EntryWhsHandler implements Handler<CommandEvent>{
	...
	private void doDisponse(Channel channel,String openid,String soatype)
	{
		if(ValidationUtils.isNotEmpty(eid))
		{
			...
			if(ValidationUtils.isNotEmpty(pd))
			{
				...
				if(ValidationUtils.isEmpty(errorMsg))
				{
					...
					//用户积分
					String entryWhsId=ValidationUtils.isNotEmpty(pd.getEntryWhsId())?pd.getEntryWhsId():"暂无";
					String entryWhsName=ValidationUtils.isNotEmpty(pd.getEntryWhsName())?pd.getEntryWhsName():"暂无";
					log.info("入职直营体验点："+entryWhsId+"|"+entryWhsName);
					String tips = rmrp.get("EntryWhsId",Constants.Platform.ALL,userName,entryWhsId,entryWhsName);
		...
		String userLoginUrl = rmrp.get("UserLoginURL",Constants.Platform.ALL,openid);
		String notbind = rmrp.get("NotBindTips",Constants.Platform.ALL,"入职直营体验点查询",userLoginUrl,userBindUrl);
